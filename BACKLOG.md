...
- **PROPOSAL (needs green light)** — [startup/mcp-gating] Decouple the MCP socket bind from canvas-storage bootstrap. `init_mcp_server` (main.rs ~3051) sits inside the `else` of `if state.bootstrap_error()` (~3019), so when canvas storage is unavailable the listener never binds and the shim gets connection-refused with no signal. Surfaced 2026-05-22: reinstalling the release `.app` resets macOS TCC, so the `open`-launched app couldn't read `~/Documents/AgentCanvas` → bootstrap_error → no socket (direct terminal launch inherits granted TCC and works fine). Proposed: always bind the socket; if canvas is unavailable, have tools return a structured "storage unavailable" error instead of the server simply not existing. Decision needed because it changes the bootstrap-error UX contract.
- todo — [install-release/restart] `scripts/install-release.sh` only `pkill`s the dev binary + vite, then `open`s — but `open` re-activates an already-running RELEASE instance instead of loading the freshly-built binary, so stale code keeps running silently (bit this session twice; the LaunchServices instance must be hard-quit). Add a `pkill -f "AgentCanvas.app/Contents/MacOS"` (or `osascript quit` + wait + force-kill fallback) before `open`, and verify the new pid actually differs.
- todo — [startup/tcc] After any release reinstall, the `.app` needs Documents-folder access re-approved (TCC resets on binary change). Document this in user-guide.md troubleshooting; consider Full Disk Access guidance or moving canvas root out of the TCC-protected Documents tree.
