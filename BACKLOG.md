...
- todo — [send-dialog] The Send-to dialog's Custom action verb persists across opens — owner saw "Can you render in html?" (the prior send's instruction) pre-filled on the next send (2026-05-22). Reset `sendActionVerb`/`customActionVerb` to the default preset (Review) on each `openSendPopover`, don't carry the last custom verb forward. Small App.tsx fix in openSendPopover (~1438-1440).
- todo — [interactive-send] Frictionless send-back for interactive HTML artifacts. When an agent generates an interactive HTML form (e.g. handoff-event-schema-decisions.html with radio choices), the only way to return the user's *in-form decisions* today is a clipboard "Copy send-back payload → paste in terminal" trick the AGENT bakes in — canvas's native Send-to only sends artifact + action verb + note, not the form state. Build: expose an MCP-backed submit through the existing HTML postMessage bridge (Slice 2) so a "Send decisions" button in the artifact posts structured form state back as a user_message — no clipboard, no terminal paste. Surfaced 2026-05-22 during live loop test. High-value for the round-trip UX.
